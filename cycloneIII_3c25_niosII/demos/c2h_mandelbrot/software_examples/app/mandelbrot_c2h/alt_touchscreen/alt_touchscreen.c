     ...
     alt_touchscreen_pen_state* ps)
{
  ...
  // Note that this update can only ever SET a flag.  The flags are
  // CLEARED (acknowledged) when their callbacks are dispatched in the
  // application-context.  They stay set until acknowledged.
  //
...
////////////////////////////////////////////////////////////////
// callback_registry_entry_unregister 
//
// Terminate event-callback activity for this entry.
//
////////////////////////////////////////////////////////////////

void callback_registry_entry_unregister (
...
////////////////////////////////////////////////////////////////
// callback_registry_unregister_callback
// 
// Un-registers the indicated function, which must be the same
// function-pointer that you passed to "_register_callback."
//
// Returns zero if the indicated callback-function was found and
// removed, nonzero otherwise.
//
////////////////////////////////////////////////////////////////
int callback_registry_unregister_callback (
		   alt_touchscreen_callback_registry*   cbr,
		   alt_touchscreen_event_callback_func  callback_func)
{
  int i;
  ...
  for (i = 0; i < ALT_TOUCHSCREEN_CALLBACK_REGISTRY_NUM_ENTRIES; i++) {
    if (cbre->callback_func == callback_func) {
      callback_registry_entry_unregister (cbre);
      return 0;   // Success.
    }
    cbre++;
  }
...
void pen_adc_data_update_from_sample_machine (alt_touchscreen_pen_adc_data* dat,
					      alt_touchscreen_sample_machine* sm)
{
  dat->x = assemble_sample ((sm->rxdata_values)[1], (sm->rxdata_values)[2]);
  dat->y = assemble_sample ((sm->rxdata_values)[3], (sm->rxdata_values)[4]);
}
...
void hardware_stop (alt_touchscreen_hardware* hw)
{
 ...
 // alt_irq_register (hw->spi_controller_irq_number, NULL, NULL);
  alt_ic_isr_register (TOUCHPANEL_SPI_IRQ_INTERRUPT_CONTROLLER_ID, hw->spi_controller_irq_number, NULL, NULL,NULL);
...
void spi_isr (void* context, alt_u32 interrupt_number)
{
  ...
  if (sample_machine_done (machine)) {
    ...
    if (pen_detect) 
      pen_adc_data_update_from_sample_machine (pen_data, machine);
...
//				 (void*) context,
//			 spi_isr);
  error_code = alt_ic_isr_register (TOUCHPANEL_SPI_IRQ_INTERRUPT_CONTROLLER_ID,hw->spi_controller_irq_number,
				 spi_isr,
				 (void*) context,
				 NULL);
  if (error_code) 
...
////////////////////////////////////////////////////////////////
// alt_touchscreen_scale
//
//  Simple coordinate-scaling routine.  Wouldn't my highschool
//  algebra teacher be proud?
//
////////////////////////////////////////////////////////////////

int alt_touchscreen_scale (unsigned int adc_value,
			   unsigned int min_adc_value,
			   unsigned int max_adc_value,
			   int          min_coordinate,
			   int          max_coordinate )
{
  int numerator = 
    (adc_value - min_adc_value) * (max_coordinate - min_coordinate);

  int denominator = (max_adc_value - min_adc_value);
  return (numerator / denominator) + min_coordinate;
}


////////////////////////////////////////////////////////////////
//
// alt_touchscreen_get_coherent_pen_data
//
...
unsigned int alt_touchscreen_get_coherent_pen_data 
    (alt_touchscreen_pen_state*       pen_state,
				      ...
				      int* y)
{
  ...
  sample_number = alt_touchscreen_get_coherent_pen_data 
    (&(screen->pen_state),
     &(screen->pen_adc_data),
     screen->swap_xy,
     pen_down,
  ...
  //
  *x = alt_touchscreen_scale (x_adc_sample, 
			      screen->x_left_adc_value,
			      screen->x_right_adc_value,
			      screen->x_left_screen_coordinate,
			      screen->x_right_screen_coordinate);

  *y = alt_touchscreen_scale (y_adc_sample, 
			      screen->y_bottom_adc_value,
			      screen->y_top_adc_value,
			      screen->y_bottom_screen_coordinate,
			      screen->y_top_screen_coordinate);
  return sample_number;
}
...
void alt_touchscreen_event_loop_update (alt_touchscreen* screen)
{
  ...
  int sample_number = alt_touchscreen_get_pen
  ...
  { 
    // Debug hackery.  Please delete.
    //printf  ("(%d)", sample_number);
    //printf  ("(%d)", screen->sample_machine.debug_sample_count);
    
    if (screen->pen_state.pen_down) {
      //printf ("[%d]", screen->sample_machine.rxdata_values[0]);
      //printf  ("(%d)", screen->sample_machine.debug_sample_count);
    }
  } 

  // This is running in application-context, so this is our only 
  // opportunity to report trouble if we detect it.
  //
  /* In my little experiments, I never saw this happen.  The SPI 
     was running at 8KHz.
     
     if (screen->sampling_running_behind_alarm) 
       printf ("Touchscreen sampling is running behind  alarm-schedule.\n");
  */
}
			  ...
			  alt_touchscreen_axis_swap_choice swap_xy)
{
  ...
  screen->x_left_adc_value            = 0;
  screen->x_right_adc_value           = 4097;
  screen->x_left_screen_coordinate    = 0;
  screen->x_right_screen_coordinate   = 4097;

  screen->y_bottom_adc_value          = 0;
  screen->y_top_adc_value             = 4097;
  screen->y_bottom_screen_coordinate  = 0; 
  screen->y_top_screen_coordinate     = 4097;
...
{ 
  screen->x_right_adc_value           = x_adc_value;
  screen->y_top_adc_value             = y_adc_value;
  screen->x_right_screen_coordinate   = x_screen_coordinate;
  screen->y_top_screen_coordinate     = y_screen_coordinate;
}
...
{ 
  screen->x_left_adc_value               = x_adc_value;
  screen->y_bottom_adc_value             = y_adc_value;
  screen->x_left_screen_coordinate       = x_screen_coordinate;
  screen->y_bottom_screen_coordinate     = y_screen_coordinate;
}
