...
#define NUM_FLASH_BLOCKS ((FLASH_INTEL_P30_NUM_32K_BLOCKS ) + \
                          (FLASH_INTEL_P30_NUM_128K_BLOCKS)   )
#define NUM_FLASH_BLOCK_BOUNDARIES ((NUM_FLASH_BLOCKS) + 1)

static 
void flash_init_block_map (alt_u32 block_map [NUM_FLASH_BLOCK_BOUNDARIES])
{
  int boundary_num  = 0;
  alt_u32 offset    = 0;

  int low_block_size  = FLASH_INTEL_P30_BOTTOM_BOOT_BLOCK ?
                         32 * 1024                        :
                        128 * 1024                        ;

  int high_block_size = FLASH_INTEL_P30_BOTTOM_BOOT_BLOCK ?
                        128 * 1024                        :
                         32 * 1024                        ;

  int num_low_blocks  = FLASH_INTEL_P30_BOTTOM_BOOT_BLOCK ?
                        FLASH_INTEL_P30_NUM_32K_BLOCKS    :
                        FLASH_INTEL_P30_NUM_128K_BLOCKS   ;

  int num_high_blocks = FLASH_INTEL_P30_BOTTOM_BOOT_BLOCK ?
                        FLASH_INTEL_P30_NUM_128K_BLOCKS   :
                        FLASH_INTEL_P30_NUM_32K_BLOCKS    ;
...
////////////////////////////////////////////////////////////////
static 
alt_u32 flash_find_previous_block_boundary (alt_u32 offset)
{
  alt_u32 block_map [NUM_FLASH_BLOCK_BOUNDARIES];
  int i;
  
  flash_init_block_map (block_map);

  for (i = 1; i < NUM_FLASH_BLOCK_BOUNDARIES; i++)
...
////////////////////////////////////////////////////////////////
static 
alt_u32 flash_find_next_block_boundary (alt_u32 offset)
{
  alt_u32 block_map [NUM_FLASH_BLOCK_BOUNDARIES];
  int i;
...
{
  alt_u32 block_start_offset = flash_find_previous_block_boundary (offset);
  int device_id_reg = 0;
				 ...
				 int  program_retry_max)
{
  ...
  while (word_count > 0) {
    ...
    int next_block_boundary = flash_find_next_block_boundary (offset);
			       ...
			       int erase_retry_max)
{
  ...
  while (remaining > 0) {
    ...
    offset = flash_find_next_block_boundary (offset + 1);
