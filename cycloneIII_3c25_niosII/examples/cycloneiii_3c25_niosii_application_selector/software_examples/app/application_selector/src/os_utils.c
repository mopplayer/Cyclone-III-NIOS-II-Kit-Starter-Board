...
void PrintfTask( void* pdata )
{
  OS_Q_DATA qdata;
  INT8U err;
  as_printf_com_message* printf_com_message;
  int length;
  ...
  {
    // Wait for a message
    printf_com_message = OSQPend( OSPrintfQ, 0, &err );
    
    if( err == OS_NO_ERR )
    {
      // Print the message
      length = strlen( printf_com_message->string );
      fwrite ( printf_com_message->string, 1, length, stdout );
      free( printf_com_message );
    }
  }
}
...
int printf( const char * format, ... )
{
  ...
  printf_com_message = malloc( sizeof( as_printf_com_message ));
  va_list ap;
  va_start( ap ,format );
    
  ret_code = vsprintf( printf_com_message->string, format, ap );
  
  // If the OSPrintfQ message queue has not yet been initialized, 
  // wait until it has before we proceed.
  while( OSQQuery( OSPrintfQ, &qdata ) != OS_NO_ERR )
  {
    OSTimeDlyHMSM( 0, 0, 0, 10 );
  }

  // If the message queue is full, wait until there is room.
  if( qdata.OSNMsgs >= qdata.OSQSize )
  {
    do
    {
      OSTimeDlyHMSM( 0, 0, 0, 10 );
      OSQQuery( OSPrintfQ, &qdata );
    } while( qdata.OSNMsgs >= qdata.OSQSize );
  }
  
  err = OSQPost( OSPrintfQ, (void *)printf_com_message );
