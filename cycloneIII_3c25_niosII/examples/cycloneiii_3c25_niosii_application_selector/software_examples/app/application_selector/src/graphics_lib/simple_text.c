...
int vid_string_pixel_length_alpha( struct abc_font_struct font[], char string[] )
{
  ...
  for( i = 0; string[i] != '\0'; i++ )
  {
    if( string[i] == ' ' )
    {
      ...
      line_length += font['-' - 33].bounds_width;
    }
    // We can handle '\n' too
    else if( string[i] == '\n' )
    {
    ...
    // ignore '\r' if we encounter it
    else if( string[i] == '\r' )
    {
      // Do nothing.
    }
    // Make sure it's a character we support
    else if( string[i] < 33 || string[i] > 126 )
    {
      // Here we just add the length of the '.' character
      line_length += font['.' - 33].bounds_width;      
    }
    ...
    {
      line_length += font[string[i] - 33].bounds_width;
    }
  }
  if( line_length > length )
  {
...
int vid_count_occurrences_in_string( char* string, char ch )
{
  int i;
  int count = 0;
  ...
  {
    count = ( string[i] == ch ) ? ( count + 1 ) : count;
  }
  return( count );
}
